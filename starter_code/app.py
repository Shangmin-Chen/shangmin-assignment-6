...
def generate_plots(N, mu, sigma2, S):
    # STEP 1: Generate X and Y with Y as a function of X plus additive normal error
    X = np.random.rand(N)  # Generates N random values between 0 and 1
    Y = mu + X + np.random.normal(0, np.sqrt(sigma2), N)  # Adjusted Y generation
    ...
    # Generate scatter plot of (X, Y) with regression line
    ...
    # Step 2: Run S simulations for histograms of slopes and intercepts
    slopes = []
    intercepts = []

    for _ in range(S):
        X_sim = np.random.rand(N)
        Y_sim = mu + X_sim + np.random.normal(0, np.sqrt(sigma2), N)  # Adjusted Y generation for simulations
